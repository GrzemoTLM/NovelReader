...
public class HelloController {
    ...
    private void openReader(BookViewModel book) {
        try {
            FXMLLoader loader = new FXMLLoader(HelloApplication.class.getResource("reader-view.fxml"));
            Scene scene = new Scene(loader.load(), 900, 700);
            ReaderController readerController = loader.getController();
            readerController.loadBook(book);
            Stage stage = (Stage) addBookButton.getScene().getWindow();
            stage.setScene(scene);
            stage.setTitle("Novel Reader - " + book.getTitle());
        } catch (IOException e) {
            showError("Błąd otwierania czytnika: " + e.getMessage());
        }
    }
    ...
    private void showBookMetadataDialog(Path filePath) {
        setLoading(true);
        statusLabel.setText("Parsowanie metadanych...");

        bookService.parseMetadata(filePath)
                .thenAccept(metadata -> Platform.runLater(() -> {
                    setLoading(false);
                    statusLabel.setText("");
                    showMetadataDialogWithData(filePath,
    ...
    private void showMetadataDialogWithData(Path filePath, String parsedTitle, String parsedAuthor, String parsedDescription) {
        ...
        TextField titleField = new TextField(parsedTitle);
        titleField.setPromptText("Wpisz tytuł...");

        Label authorLabel = new Label("Autor:");
        TextField authorField = new TextField(parsedAuthor);
        authorField.setPromptText("Wpisz autora...");

        Label descriptionLabel = new Label("Opis:");
        TextArea descriptionField = new TextArea(parsedDescription);
